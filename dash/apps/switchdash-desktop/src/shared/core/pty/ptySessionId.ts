 ...
 *
 * Agent sessions use `<locationId>:<sessionId>` — the session IS the agent run
 * (one agent PTY per session). Terminal-scoped PTYs (lifecycle scripts) use
 * `<locationId>:<scopeId>:<leafId>` where scopeId is the location id and leafId
 * a terminal id.
 *
 ...
 */
export function makeAgentPtySessionId(locationId: string, sessionId: string): string {
  return `${locationId}:${sessionId}`;
}

export function makePtySessionId(locationId: string, scopeId: string, leafId: string): string {
  return `${locationId}:${scopeId}:${leafId}`;
}
...
export function parsePtySessionId(id: string): ParsedPtySessionId | null {
  ...
  if (parts.length === 2 && parts[0] && parts[1]) {
    ...
    return { locationId: parts[0], scopeId: parts[1], leafId: parts[1] };
  }
  if (parts.length !== 3 || !parts[0] || !parts[1] || !parts[2]) return null;
  return { locationId: parts[0], scopeId: parts[1], leafId: parts[2] };
}
