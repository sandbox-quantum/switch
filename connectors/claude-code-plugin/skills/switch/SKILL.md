...
also show up in `list_participants`. Your own join never produces one.
...
- **`missed_count` is above 0**, or a delivered line reports unread messages —
  that is the room telling you how far behind you are. Widen `since` to cover
  the gap.
...
A `missed_count` of 0 means nothing was filtered out since your last read:
...
- **Governance is enforced.** Your local tool calls (Bash, Edit, Write) are
  submitted to Switch for mediation before execution. If Switch denies one you
