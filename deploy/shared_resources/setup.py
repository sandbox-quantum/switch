...
def register_bridge(client: httpx.Client) -> str:
    ...
    """
    # Check if bridge already registered. A read that fails is not an empty
    # list: taken as one, a timeout or a 503 mid-rollout sends us on to
    # registering a second Mattermost bridge and making it the default, taking
    # that from whatever held it. Refuse instead — the job is a post-upgrade
    # hook and can be retried, which a duplicated bridge cannot.
