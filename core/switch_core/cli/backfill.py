...
# The probe answers "is there a homeserver at all", not "is it healthy", so it
# is short: a slow answer and no answer lead to the same skip.
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do nothing unless there is a reachable homeserver and history "
            "that has never been walked. For the unattended path, where the "
            "answer to 'is this deployment mid-migration?' has to be worked "
            "out rather than known. Implies --allow-empty."
        ),
    )
    parser.add_argument(
        "--force",
...
async def _homeserver_is_up(server_url: str) -> bool:
    """Whether there is still a homeserver at `server_url`.

    A deployment that never had one, or that has already taken it down, has
    nothing to copy and must not be held up saying so. Unreachable is treated
    as absent on purpose: this only ever guards work that is safe to skip and
    safe to repeat, so the worst a wrong answer costs is one more run.
    """
    try:
        async with aiohttp.ClientSession() as http:
            async with http.get(
                f"{server_url.rstrip('/')}/_matrix/client/versions",
...
async def _run(args: argparse.Namespace) -> int:
    ...
    if args.if_needed:
        ...
        if not await _homeserver_is_up(config.matrix_server):
            print(
                f"No homeserver answering at {config.matrix_server}; there is "
                "nothing to copy. Skipping.",
                file=sys.stderr,
            )
            return 0
    ...
    transport = MatrixTransport(
        ...
        # Logs in fresh. The stored device id and access token went with the
        # transport that used them; a one-off command can afford the round
        # trip, and it is the only thing left that needs an account at all.
        device_id=None,
        access_token=None,
    )
