...
**The table list is derived, not copied.** A table is scoped if it carries a
...
declarative class. Deriving it here means a table added later either follows
the pattern and is picked up automatically, or does not and is exempt from
these policies while still being visible as a gap to the catalogue test in
`tests/switch_core/db/test_row_level_security.py` — never silently skipped by
a list someone forgot to extend.
...
def scoped_tables(metadata: MetaData) -> dict[str, str]:
    """Table name -> the column its policy compares, derived from the schema.

    `tenants` is the one table scoped by `id`: every other table is scoped by
    carrying a `tenant_id` column whose foreign key targets `tenants.id`. That
    second test — the foreign key, not just the column name — is deliberate:
    a table that merely happens to have a column called `tenant_id` without
    the constraint is not actually tied to a tenant and must not silently pick
    up a policy that would then reject every row it holds.
    """
    tables: dict[str, str] = {}
