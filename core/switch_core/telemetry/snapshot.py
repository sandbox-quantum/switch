...
def _human_interaction(since: datetime) -> Select[tuple[str]]:
    ...
    return (
        ...
        .where(
            ...
            exists(room_has_an_agent),
        )
    )


def _active_humans(since: datetime) -> Select[tuple[str | None]]:
...
async def collect_tenant_counts(
    session: AsyncSession, counts: UsageCounts, now: datetime
) -> None:
    """Add one tenant's numbers to `counts`.

    The session must already be bound to that tenant: every query here relies
    on row-level security to narrow it, rather than naming the tenant itself.
    """
    day_ago = now - _DAY
    week_ago = now - _WEEK

    counts.user_active_1d += await _count(session, _active_humans(day_ago))
    counts.user_active_7d += await _count(session, _active_humans(week_ago))
    counts.room_active_1d += await _count(session, _human_interaction(day_ago))
    counts.room_active_7d += await _count(session, _human_interaction(week_ago))

    # Rooms, split by who made them. `created_by_kind` is stamped into the
    # room's metadata at creation; rooms made before that existed carry
    # nothing, and are counted as user-created — the conservative reading,
    # since the agent-created room is the newer path.
    agent_made = Room.metadata_["created_by_kind"].astext == "agent"
    counts.room_count += await _scalar(
        session,
        select(func.count())
        .select_from(Room)
        .where(Room.archived_at.is_(None), ~agent_made | agent_made.is_(None)),
    )
    counts.room_agent_created_count += await _scalar(
        session,
        select(func.count())
        .select_from(Room)
        .where(Room.archived_at.is_(None), agent_made),
    )
    counts.room_archived_count += await _scalar(
        session,
        select(func.count()).select_from(Room).where(Room.archived_at.is_not(None)),
    )
    counts.room_internal_only_count += await _scalar(
        session,
        select(func.count())
        .select_from(Room)
        .where(Room.archived_at.is_(None), Room.bridge_id.is_(None)),
    )
    ...
    per_room = (
        ...
        .where(Client.type == HUMAN_CLIENT_TYPE, Room.archived_at.is_(None))
    ...
    by_runtime = await session.execute(
        select(runtime, func.count()).select_from(Agent).group_by(runtime)
    )
    ...
    counts.agent_active_7d += await _count(
        ...
        select(distinct(Message.sender_client_id))
        .join(Client, Client.id == Message.sender_client_id)
        .where(Message.sent_at >= week_ago, Client.type == AGENT_CLIENT_TYPE),
    )
    ...
    counts.message_count_1d += await _scalar(
        ...
        select(func.count())
        .select_from(Message)
        .where(Message.sent_at >= day_ago, Message.seq > 0),
    )
    ...
    await _collect_turns(session, counts, day_ago)
...
async def _collect_turns(
    session: AsyncSession, counts: UsageCounts, since: datetime
) -> None:
...
async def room_had_human_activity(session: AsyncSession, room_id: str) -> bool:
    """Whether a human has ever posted in this room.

    The session must already be bound to the room's tenant.
    """
...
async def collect_usage(
    ...
    for tenant_id in tenant_ids:
        async with tenant_session(session_factory, tenant_id) as session:
            await collect_tenant_counts(session, counts, moment)
    ...
    for tenant_id in await all_tenant_ids(session_factory):
        async with tenant_session(session_factory, tenant_id) as session:
            first_interaction = (
                ...
                .join(Client, Client.id == Message.sender_client_id)
                .where(Client.type == HUMAN_CLIENT_TYPE, Message.seq > 0)
