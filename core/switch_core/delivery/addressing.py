...
class AddressingResolver:
    ...
    async def addresses(
        ...
        **A system message never addresses anyone**, whatever the room type.
        ...
        if self.mentions_name(
            ...
            return True
        if await self.mentions_alias(agent=agent, room_id=room_id, message=message):
            return True
        return await self.mentions_role(agent=agent, room_id=room_id, message=message)
    ...
    async def mentions_alias(
        self, *, agent: Agent, room_id: str, message: IncomingMessage
    ) -> bool:
    ...
    async def permitted(
        self, *, agent: Agent, room_id: str, sender: str
    ) -> AddressingDecision:
