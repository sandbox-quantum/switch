...
class RoomYamlService:
    ...
    async def export(
        ...
        *,
        ...
        """
        async with self._session_factory() as session:
            data = await self._export_room_data(
                session,
                room_id,
                agents=agents,
                users=users,
    ...
    async def _export_room_data(
        ...
        *,
        ...
        """Build the room data dict for a single room. Shared by ``export``
        and ``export_group``."""
        ...
        if users and room.bridge_id:
            ...
            client_to_name = {eu.client_id: eu.external_username for eu in ext_users}
            user_names = sorted(
    ...
    async def export_group(self, group_id: str) -> str:
        """Export all rooms in a group to the group document shape.

        Emits ``group:`` + ``rooms:`` + ``links:`` — the same surface
        ``parse`` accepts for group provisioning.
        """
        from switch_core.db.models import Room

        async with self._session_factory() as session:
            group = await self._room_groups.get(session, group_id)
            if group is None:
                raise ValueError(f"Room group not found: {group_id}")

            result = await session.execute(
                sa_select(Room).where(Room.group_id == group_id)
            )
            rooms = list(result.scalars().all())
            if not rooms:
                raise ValueError(f"Room group {group_id!r} has no rooms")

            room_data_list: list[dict[str, Any]] = []
            room_id_to_name: dict[str, str] = {}
            for room in rooms:
                data = await self._export_room_data(session, room.id)
                room_data_list.append(data)
                room_id_to_name[room.id] = data["name"]

            room_ids = set(room_id_to_name.keys())
            links_data: list[dict[str, str]] = []
            for rid in room_ids:
                outbound = await self._resources.list_linked_rooms_for_room(
                    session, rid
                )
                for link in outbound:
                    target_id = link["target_room_id"]
                    if target_id in room_ids:
                        links_data.append(
                            {
                                "from": room_id_to_name[rid],
                                "to": room_id_to_name[target_id],
                                "label": link["label"],
                            }
                        )

        group_meta: dict[str, Any] = {"name": group.name}
        if group.description:
            group_meta["description"] = group.description
        if group.color:
            group_meta["color"] = group.color
