...
class MatrixTransport:
    ...
    async def receive_forever(self) -> None:
        """Not a receiver any more.

        Nothing in Switch runs on this transport; it survives so the backfill
        and reconcile commands can still read the homeserver's history. Those
        call `read_history`, never this. Raising rather than syncing says so at
        the moment someone wires it back into a client by mistake.
        """
        raise NotImplementedError(
            "The Matrix transport no longer delivers events. It is kept for the "
            "backfill and reconcile commands, which read history and nothing else."
        )
