...
async def list_api_keys(
    ...
    return [
        ApiKeyDetail(
            ...
            label=k.label,
            type=k.type,
            # The plaintext opening of the key, so an operator holding a key
            # can match it against the row. `key_hash[:12]` is a prefix of the
            # sha256 digest, not of the key — it matches nothing the operator
            # can see. Decrypting here mirrors what `reveal_api_key` does.
