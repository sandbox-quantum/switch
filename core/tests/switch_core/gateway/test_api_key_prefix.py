...
"Key Prefix" column — truthfully a hash prefix, but displayed as though it
were the key's opening characters, so an operator holding a key could not
match it against its row.

Driven over real HTTP against real Postgres, in the style of
`test_change_password.py`: the value has to survive create → store → list, and
a stub for any of those steps would pass on a prefix taken from either string.
"""
