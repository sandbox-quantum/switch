...
async def test_refresh_cookie_max_age_tracks_jwt_expiry() -> None:
    """The cookie's Max-Age must be derived from JWT_EXPIRY_HOURS, not a
    hardcoded literal — otherwise the two drift apart silently, giving
    either premature logouts or a cookie outliving its own token."""
