...
function controlledEnvironment(root: string): Record<string, string> {
  return {
    HOME: join(root, 'home'),
      ...
      JSON.stringify({
        ...
        config: buildSharedHostConfig({
          session: {
            ...
            ...(spec.session.nativeSessionId
              ? { nativeSessionId: spec.session.nativeSessionId }
              : {}),
          },
          launch: {
            cwd: workspaceRoot,
            ...
            env: environment,
            ...(spec.provider.model ? { model: spec.provider.model } : {}),
          },
...
const defaultDependencies: HostedBootstrapDependencies = {
...
export async function runHostedBootstrap(
  ...
  dependencies: HostedBootstrapDependencies = defaultDependencies
): Promise<void> {
  ...
  try {
    await dependencies.supervise({
      ...
      fenceDeadWorker: dependencies.fenceDeadWorker,
