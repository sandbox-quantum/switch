...
its own provider adapter instance.
...
one host for the whole agent. The supervisor/worker pair per room is what the
change removes; provider and MCP subprocesses stay where the provider requires
them, so measure the real number against the provider under test.

## Containing one session's failure

Nothing kills a process group any more. That was the old teardown, and in a
resident host it would take every sibling room with it. Instead each room
session tears down only what is its own:

- its provider and MCP children, through that session's own adapter instance;
...
room session, records the reason in that session's `supervisor/failure.json`
and in the host's `resident.json`, and leaves the host and the other rooms
running. A fault in the host itself is still a shared failure, and is visible as
one.
...
sessions claiming one room is refused with an error naming both.

Switch allows 32 connections per agent. Discovery spends one, each room session
one more. The local budget of 31 is an early warning only — other clients of the
same agent hold slots too — so the authority is the server's own refusal: a room
that is not admitted inside the admission window fails the session with the
server's message quoted, rather than sitting behind a stream that retries for
ever.

## The larger alternative

One multi-room connection per agent would drop the per-room connection entirely:
one heartbeat, one cursor, no connection budget, and a single stream to recover.
It needs server changes — the connection-to-session binding, one connection ↔ one
session, and the per-session fencing that hangs off it all assume a single-room
...
- No idle policy. A room session runs until it is stopped, faults, or the host
  goes down; nothing retires a quiet room to free its connection.
