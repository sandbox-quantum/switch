 ...
 *    no longer exists.
 * 4. Only when `removeProvisionedFiles` is set, or a remote agent is deleted
 *    in Switch: the Switch credentials + definition file provisioned on disk
 *    for THIS agent (local or remote), and its sidecar. A plain remove leaves
 *    the working directory and the host's processes untouched — on a shared
 *    host they may belong to another install (CHOO-2560). Sibling agents'
