...
async function tenantToAdopt(
  ...
  // Only where there is nothing to choose between. Nothing was ever registered
  // through the row, so there is no data to put anywhere, and a membership the
  // user can see and switch away from beats a row belonging to none of them.
  // With several it would be a coin toss deciding which membership inherits the
  // id the saved navigation and the active selection point at; left unadopted,
  // every membership gets its own row and the empty placeholder is dropped
  // further down, which reaches the same list without the toss.
  if (unmatched.length === 1 && !(await workspaceHasAgents(unclaimed.id))) return unmatched[0]!;
  return null;
}
...
export async function reconcileServerWorkspaces(serverId: string): Promise<void> {
  ...
  const queued = (inFlight.get(serverId) ?? Promise.resolve())
    .then(
    ...
    // goes on being offered in the switcher after the row it names has gone.
    .then(() => {
      events.emit(workspacesChangedChannel, undefined);
    });
  inFlight.set(
    serverId,
    queued.then(
