...
export const workspacesController = createRPCController({
  ...
  getAgent: (params: { workspaceId: string; agentId: string }): Promise<RemoteAgentSummary> =>
    withWorkspaceSession(params.workspaceId, (server) => fetchAgentDetail(server, params.agentId)),
  ...
  listTemplates: (params: {
    ...
    withWorkspaceSession(params.workspaceId, (server) => fetchTemplates(server, params.kind)),
  ...
  backfillAgentIcons: (workspaceId: string): Promise<AgentIconBackfill> =>
    withWorkspaceSession(workspaceId, backfillAgentIcons),
  ...
  /**
   * Register a new agent in the chosen workspace (owned by the signed-in user)
   * and write its credentials into the directory's `.claude/settings.local.json`.
   ...
   */
  provisionAgent: (params: ProvisionAgentParams): Promise<ProvisionAgentResult> =>
    withWorkspaceSession(params.workspaceId, async (server) => {
   ...
   */
  provisionRemoteAgent: (params: ProvisionRemoteAgentParams): Promise<ProvisionAgentResult> =>
    withWorkspaceSession(params.workspaceId, async (server) => {
      ...
      const proxy = await ensureSshConnected(
        sshConnectionIdForHost(params.sshHost),
        params.sshHost
      );
