...
async function requireAddressableWorkspace(workspaceId: string): Promise<Workspace> {
  ...
  if (onServer.length > 1) {
    throw new Error(
      `This workspace has not been matched to one of the ${onServer.length} this account belongs to on its Switch server, so there is no way to tell which one to ask. Sign in to the server again to match it.`
    );
  }
  return workspace;
}
