/**
 * Migration 0051 — introduces workspaces, the unit the window is scoped to, and
 ...
 * exists.
 *
 * The SQL is applied to a hand-built pre-0051 schema (the same approach as the
...
describe('migration 0051: workspaces', () => {
  ...
  beforeEach(() => {
    ...
    db.exec(`
      ...
      -- Mirrors the real pre-0051 agents DDL, foreign keys included. The
  ...
  it('carries every other agent column through the table rebuild', () => {
    ...
    db.prepare(
      `INSERT INTO agents (id, location_id, name, provider_id, switch_agent_id, api_endpoint, server_id, status, auto_approve, owner_name, provider_config)
       VALUES ('agent-a', 'loc-1', 'Worker', 'codex', 'switch-123', 'https://api.example', 'srv-local', 'live', 1, 'owner-1', '{"v":1}')`
    ).run();
