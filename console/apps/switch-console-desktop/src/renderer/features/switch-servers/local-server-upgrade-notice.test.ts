...
vi.mock('./local-server-store', async () => {
...
vi.mock('./switch-servers-store', async () => {
  const { observable } = await import('mobx');
  return { switchServersStore: observable({ activeServerId: 'local' }) };
});
const { localServerStore } = await import('./local-server-store');
const { switchServersStore } = await import('./switch-servers-store');
const { LocalServerUpgradeNotice } = await import('./LocalServerUpgradeNotice');
