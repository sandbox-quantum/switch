...
vi.mock('@renderer/features/switch-servers/switch-servers-store', () => ({
  switchServersStore: {
    ...
    // Stepping back off the picker draws the sign-in page, which reads these.
...
describe('picking a workspace after signing in', () => {
  ...
  it('goes back by a button and not only by a chevron', async () => {
    // The chevron is unlabelled, so it may repeat the way back but never be the
    // only one — the same invariant the who-runs page keeps.
    resolveWorkspaces.mockResolvedValue([workspace('ws-1', 'Acme'), workspace('ws-2', 'Skunk')]);
  ...
  /**
   * Leaving while a workspace is being entered lands the user two pages on when
   * it resolves — back on the sign-in, and then thrown forward out of it.
   */
  it('holds the way back shut while a workspace is being opened', async () => {
    resolveWorkspaces.mockResolvedValue([workspace('ws-1', 'Acme'), workspace('ws-2', 'Skunk')]);
    const opening = deferred<void>();
    setActiveWorkspace.mockReturnValue(opening.promise);

    const el = await renderAtPickWorkspace();
    await click(el, 'Acme');

    const back = [...el.querySelectorAll<HTMLButtonElement>('button')].find(
      (b) => b.textContent?.trim() === 'Back'
    )!;
    expect(back.disabled).toBe(true);
    const chevron = el.querySelector<HTMLButtonElement>('[aria-label^="Previous page"]')!;
  ...
  /**
   * The workspace exists on the server from the moment Create returns. Asking
   * again would send back a name it has just taken and earn a conflict that
   * reads as the name being refused — and on this page, reached because the
   * account is in no workspace at all, there is no Back either.
   */
  it('offers to open the workspace it already made rather than creating it twice', async () => {
    ...
    setActiveWorkspace.mockResolvedValue(undefined);
    await click(el, 'Open workspace');
