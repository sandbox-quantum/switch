...
| OpenCode connector | `connectors/opencode-plugin/package.json` | any change to the connector; it is published, and the version is what an install resolves. Publishing it is a tag — `switch-connector-opencode-v<version>` — so the same lag rule as the runtime applies: a pin must name a version that exists on the registry. `just artifacts-check` fails if the number disagrees with `artifacts.yaml` |
