...
class Reply:
    ...
    def __exit__(self, *args):
        pass
    def read(self):
        return b'{"id":"new-watcher-id","api_key":"PLACEHOLDER_RETURNED_CREDENTIAL"}'


class RegistrationTests(unittest.TestCase):
    def test_private_save_no_overwrite_and_no_auto_session(self):
        with tempfile.TemporaryDirectory() as temp:
            output = Path(temp) / 'watcher.json'
            with patch.object(r, 'build_opener') as build:
                build.return_value.open.return_value = Reply()
                result = r.register('https://switch.invalid', 'watcher.example', output, 'PLACEHOLDER_REGISTRATION_CREDENTIAL')
                ...
                with self.assertRaises(FileExistsError):
                    r.register('https://switch.invalid', 'watcher.example', output, 'PLACEHOLDER_REGISTRATION_CREDENTIAL')
    ...
    def test_non_loopback_http_rejected_before_network(self):
        with tempfile.TemporaryDirectory() as temp, patch.object(r, 'build_opener') as build:
            with self.assertRaises(ValueError):
                r.register('http://remote.invalid', 'watcher', Path(temp) / 'watcher.json', 'PLACEHOLDER_UNUSED_CREDENTIAL')
